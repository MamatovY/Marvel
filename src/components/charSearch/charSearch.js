...
const CharSearch = () => {
    ...
    const { getCharacterFind, clearError, process, setProcess } = useMarvelService()


    const getCharFind = (name) => {
        clearError()
        getCharacterFind(name)
            .then(onCharFind)
    ...
    const errorMessage = process === 'error' ? <div className="charSearch__msg error">Error</div> : null
